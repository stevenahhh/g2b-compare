import type { ProductAttribute } from "./models";

export function conciseSpec(
  spec: string,
  ...
  if (/영상감시장치|보안용카메라/u.test(name)) {
    const shape = /PTZ|회전|스피드돔/iu.test(matchText)
      ? "회전형"
      : /Bullet|고정/iu.test(matchText)
        ? "고정형"
        : "";
    const megapixels = matchText.match(/(\d+(?:\.\d+)?)\s*MP/iu)?.[1];
    const resolution = matchText.match(/(\d+(?:\.\d+)?)\s*만화소/u)?.[1]
      ?? (megapixels ? String(Number(megapixels) * 100) : "");
    const zoom = matchText.match(/(?:광학\s*|Optical\s*x\s*)(\d+(?:\.\d+)?)\s*배?줌?/iu)?.[1]
      ?? matchText.match(/(\d+(?:\.\d+)?)\s*배줌/u)?.[1];
    if (shape || resolution || zoom) {
      return [shape, resolution && `${resolution}만화소`, zoom && `${zoom}배줌`]
        .filter(Boolean)
        .join(", ");
    }
    const cameraParts = rawSpec
      .split(",")
      .map((part) => part.trim())
      .filter(Boolean);
    return cameraParts.length >= 4
      ? cameraParts.slice(-2).join(", ")
      : rawSpec.trim();
  }
...
export function documentItemName(itemName: string, spec = ""): string {
  const name = String(itemName ?? "");
  if (!/^(?:추가선택품목|선택품목|옵션)$/u.test(name)) return name;
  return String(spec).match(/^\[[^\]]+\]\s*\[\d{8}\]\s*([^,]+)/u)?.[1]?.trim() || name;
}

export function documentUnit(itemName: string, sourceUnit: string, spec = ""): string {
...
export function compactCompanyName(value: string): string {
  return String(value ?? "")
    .replace(/^(?:주식회사|\(주\)|㈜)\s*/u, "")
    .trim();
}
